...
class ExportMode {
	...
	constructor(type) {
		...
		// for "normal-reexport" | "dynamic-reexport" | "empty-start"
		/** @type {Set<Module>} */
...
class HarmonyExportImportedSpecifierDependency extends HarmonyImportDependency {
	...
	getMode(moduleGraph, runtime) {
		...
		console.warn(this.request, exports);
		const dependencies = new Set(
			Array.from(this.otherStarExports, dep =>
		...
		switch (mode.type) {
			...
			case "dynamic-reexport": {
				...
				return {
					...
					dependencies: [from.module, ...mode.dependencies]
				};
			}
			case "empty-star":
				return {
					exports: [],
					dependencies: [moduleGraph.getModule(this), ...mode.dependencies]
				};
			case "normal-reexport": {
				const from = moduleGraph.getConnection(this);
				return {
